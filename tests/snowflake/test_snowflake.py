import pytest
from iden.snowflake.generator import SnowflakeGenerator, Snowflake, MAX_WORKER_ID, MAX_DATACENTER_ID, SEQUENCE_MASK
import time
...
def test_snowflake_worker_id_validation():
    ...
    """
    with pytest.raises(ValueError, match=f"Worker ID must be between 0 and {MAX_WORKER_ID}"):
...
def test_snowflake_datacenter_id_validation():
    ...
    """
    with pytest.raises(ValueError, match=f"Datacenter ID must be between 0 and {MAX_DATACENTER_ID}"):
...
def test_snowflake_class_datacenter_id_validation():
    ...
    """
    with pytest.raises(ValueError, match=f"Datacenter ID must be between 0 and {MAX_DATACENTER_ID}"):
        Snowflake(timestamp=1, worker_id=1, datacenter_id=MAX_DATACENTER_ID + 1, sequence=0)
    with pytest.raises(ValueError, match=f"Datacenter ID must be between 0 and {MAX_DATACENTER_ID}"):
        Snowflake(timestamp=1, worker_id=1, datacenter_id=-1, sequence=0)
